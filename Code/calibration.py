...
def dectshow(org_img, boxs,depth_data,key,ref):
    img = org_img.copy()
    for index, box in enumerate(boxs):
        cv2.rectangle(img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 255, 0), 2)
        dist = get_mid_pos(org_img, box, depth_data, 24)
        cv2.putText(img, 'Index: ' + str(index) + ' ' + box[-1] + str(dist / 1000)[:4] + 'm',
    ...
    if(key == ord('s')):
        if(len(boxs) == 0):
            ...
            print('Which box do you want to choose?')
            choose = input()
            choose = int(choose)
            if(choose >= len(boxs)):
                print('Out of range!')
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            dectshow(color_image, boxs, depth_image, key0, ref)
            print(ref)
